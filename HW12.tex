...
\hlcom{#run t-test for 20 observations}
\hldef{sim20} \hlkwb{<-} \hldef{sim[}\hlnum{1}\hlopt{:}\hlnum{20}\hldef{]}
\hldef{t20} \hlkwb{<-} \hlkwd{t.test}\hldef{(sim20,} \hlkwc{mu} \hldef{= mu0,} \hlkwc{alternative} \hldef{=} \hlsng{"greater"}\hldef{)}

\hlcom{#check if we make type 1 error}
\hlkwa{if} \hldef{(t20}\hlopt{$}\hldef{p.value} \hlopt{<} \hlnum{0.05}\hldef{)\{}
  \hldef{type1.count} \hlkwb{=} \hldef{type1.count} \hlopt{+}\hlnum{1}
\hldef{\}}\hlkwa{else}\hldef{\{} \hlcom{#if p >= 0.05, try t-test on 30 observations}
  \hldef{t30} \hlkwb{<-} \hlkwd{t.test}\hldef{(sim,} \hlkwc{mu} \hldef{= mu0,} \hlkwc{alternative} \hldef{=} \hlsng{"greater"}\hldef{)}
  \hlkwa{if} \hldef{(t30}\hlopt{$}\hldef{p.value} \hlopt{<} \hlnum{0.05}\hldef{)\{}
    \hldef{type1.count} \hlkwb{=} \hldef{type1.count} \hlopt{+}\hlnum{1}
  \hldef{\}}
\hldef{\}}
\hldef{\}}
...
\end{kframe}
\end{knitrout}
The Type I error rate for a Laplace distribution with $a=0$ and $b=4.0$ by using a simulation is 0.068 = 6.8\%. 
\end{enumerate}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Question 2
...
\end{kframe}
\end{knitrout}
We make an error of Type I for a left-tailed test 0.034 = 3.4\% for Beta(10,2),  0.075 = 7.5\% for Beta(2,10), and 0.051 = 5.1\% for Beta(10,10).
    \item What proportion of the time do we make an error of Type I for a
    right-tailed test?
\begin{knitrout}\scriptsize
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
...
\end{kframe}
\end{knitrout}
We make an error of Type I for a right-tailed test 0.077 = 7.7\% for Beta(10,2),  0.021 = 2.1\% for Beta(2,10), and 0.045 = 4.5\% for Beta(10,10).
    \item What proportion of the time do we make an error of Type I for a
    two-tailed test?
\begin{knitrout}\scriptsize
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
